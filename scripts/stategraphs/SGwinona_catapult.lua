...
local states =
{
    State{
        ...
        onenter = function(inst)
            ...
            inst.SoundEmitter:PlaySound("winona_items/catapult/place")
            inst:AddTag("NOCLICK")
    ...
    State{
        ...
        onenter = function(inst)
            inst.SoundEmitter:PlaySound("winona_items/catapult/hit", nil, .5)
            inst.SoundEmitter:PlaySound("winona_items/catapult/ratchet_LP", "power")
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(8 * FRAMES, function(inst) inst.SoundEmitter:PlaySound("winona_items/catapult/hit", nil, .35) end),
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.SoundEmitter:PlaySound("winona_items/catapult/hit")
        end,
    ...
    State{
        ...
        onenter = function(inst, target)
            ...
            inst.SoundEmitter:PlaySound("winona_items/catapult/ratchet_LP", "attack_pre")
        end,

        onupdate = function(inst)
            if inst.sg.statemem.target ~= nil then
                --print(inst.sg.statemem.target)
                --print(inst.sg.statemem.target == GetDebugEntity())
        ...
        timeline =
        {
            TimeEvent(20 * FRAMES, function(inst)
                inst.SoundEmitter:PlaySound("winona_items/catapult/fire")
            end),
            TimeEvent(21 * FRAMES, function(inst)
                ...
                if pos == nil then
                    ...
                    local dx = pos.x - x --计算目标点和弹弓之间的坐标差
                    local dz = pos.z - z
                    local l = math.sqrt(dx * dx + dz * dz)
                    if l < TUNING.WINONA_CATAPULT_MIN_RANGE then
                        l = TUNING.WINONA_CATAPULT_MIN_RANGE / l
                        pos.x = x + dx * l
                        pos.z = z + dz * l --如果距离过近, 修正为l
                    end
                end
                pos.y = 0
                print(pos, inst.sg.statemem.target:GetPosition())
                inst.sg.statemem.target = nil --stop onupdate
                inst:ForceFacePoint(pos)
                inst.sg.statemem.rock.components.catapult_complexprojectile:Launch(pos, inst)
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.SoundEmitter:PlaySound("winona_items/catapult/destroy")
        end,
    },
